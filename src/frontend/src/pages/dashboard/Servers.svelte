	...
	async function handleAdd(event: SubmitEvent) {
		try {
			if ((<HTMLButtonElement | null>event.submitter)?.value !== 'cancel') {
				await serversStore.addServer(newServername, newPassword);
			}
		} finally {
...
<dialog class="modal" bind:this={addDialog}>
	<div class="modal-box">
		...
		<form onsubmit={handleAdd} method="dialog" class="fieldset p-4">
			<label for="servername" class="label">Servername</label>
			<input id="servername" type="text" class="input" bind:value={newServername} required />
